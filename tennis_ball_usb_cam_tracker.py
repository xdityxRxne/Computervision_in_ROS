...
def draw_ball_contour(binary_image, rgb_image, contours):
    ...
    if (area>100): # area of contours in order to perceive the ball and ot random spots maybe changes whenever there are many colours
...
def ball_detection_per_frame(frame):
...
def callback(msg):
    ...
    try:
        ...
        print(e) # shownig all video processed or not for tracking the ball
        cv2.waitKey(3)
        ball_detection_per_frame(cv_image)
...
def main(args):
    rospy.init_node('image_track_subscriber', anonymous=True)
    img_sub = rospy.Subscriber("/usb_cam/image_raw",Image,callback)
    rate = rospy.Rate(1/0.033)
    try:
        rospy.spin()
